...
class CoinListCell: UICollectionViewCell {
    ...
    func configViews() {
        ...
        for item in [coinDataStackView, border, coinValueStackView] {
            container.addSubview(item)
        }

        for item in [coinImage, nameLabel, symbolLabel, priceLabel] {
            coinDataStackView.addArrangedSubview(item)
        }

        for item in [oneHrStackView, twentyFourHrStackView, sevenDayStackView] {
            coinValueStackView.addArrangedSubview(item)
        }

        for item in [oneHrLabel, oneHourHistory, fillerone] {
            oneHrStackView.addArrangedSubview(item)
        }

        for item in [twentyFourHourLabel, twentyFourHourHistory, fillertwo] {
            twentyFourHrStackView.addArrangedSubview(item)
        }

        for item in [sevenDayLabel, sevenDayHistory, fillerthree] {
            sevenDayStackView.addArrangedSubview(item)
        }
    }
